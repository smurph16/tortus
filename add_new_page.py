...
from accessing_groups import retrieve_members
...
def add_new_page(file_path, args, name):
	#Read from file, process text and create the page
...
def process_user_link(page_name, args):
		user_ids = None
		if args.show_to_all:
...
def add_quick_link(uid, name):
	#group pages added by default
	#command line arguments
	u = user.User(request, uid)
	if not u.isQuickLinkedTo(name): #may need to be a list
		u.addQuicklink(name)
