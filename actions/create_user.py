...
the_user.email = "ExampleUser@example.com"
the_user.enc_password = user.encodePassword(request.cfg, "zoo0Eifi")
if (user.isValidName(request, the_user.name) and not user.getUserId(request, the_user.name) and not user.get_by_email_address(request, the_user.email)):
	print "The user with the userid %s doesn't exist yet" % the_user.id 
	the_user.save()
